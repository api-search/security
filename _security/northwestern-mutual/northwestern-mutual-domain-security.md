---
description: ''
domains:
- caa: []
  dmarc: true
  dmarc_policy: reject
  dnssec: false
  domain: northwestern-mutual.com
  spf: true
hosts:
- host: www.northwestern-mutual.com
  https: false
kind: domain-security
layout: security
method: probed
name: Northwestern Mutual Domain Security
name_suffix: Domain Security
overview: 'Domain security posture for northwestern-mutual, probed live across 1 host(s) and 1 registrable domain(s). Email/DNS controls: DNSSEC absent, SPF present, DMARC present (p=reject).'
provider_name: northwestern-mutual
provider_slug: northwestern-mutual
slug: northwestern-mutual-domain-security
source_filename: northwestern-mutual-domain-security.yml
source_heading: Domain Security
source_url: ''
source_yaml: "generated: '2026-07-11'\nmethod: probed\nsource: live DNS/TLS/HTTP probes of apis.yml + OpenAPI hosts\nhosts:\n- host: www.northwestern-mutual.com\n  https: false\ndomains:\n- domain: northwestern-mutual.com\n  dnssec: false\n  caa: []\n  spf: true\n  dmarc: true\n  dmarc_policy: reject\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/northwestern-mutual/refs/heads/main/security/northwestern-mutual-domain-security.yml
summary_line: DMARC
tags:
- Fortune 500
---
