...
scheme_count: 1
schemes:
- description: 'Bearer token generated in the Market Data dashboard. Passed as `Authorization: Bearer [example key]`. The token may also be supplied via a `token` query parameter for quick browser or spreadsheet use. A single IP address per account is enforced.'
  name: bearerAuth
  scheme: bearer
  sources:
