...
- authorizationCode
- implicit
overview: Snowflake secures its APIs with http, oauth2, and unknown across 4 declared security schemes, as derived from its OpenAPI definitions. OAuth 2.0 is offered via the authorizationCode and implicit flow(s).
...
- name: KeyPair
  ...
  type: unknown
- bearerFormat: JWT
  description: Key pair authentication using a JWT token. Set X-Snowflake-Authorization-Token-Type to KEYPAIR_JWT.
  name: keyPair
  scheme: bearer
  sources:
