---
description: ''
domains:
- caa: []
  dmarc: true
  dmarc_policy: none
  dnssec: true
  domain: sjtu.edu.cn
  spf: true
hosts:
- cert_expires: Oct 18 23:59:59 2026 GMT
  host: en.sjtu.edu.cn
  hsts: false
  https: true
  tls_version: TLSv1.2
- cert_expires: Oct 18 23:59:59 2026 GMT
  host: developer.sjtu.edu.cn
  hsts: false
  https: true
  tls_version: TLSv1.2
- cert_expires: Oct 18 23:59:59 2026 GMT
  host: jaccount.sjtu.edu.cn
  hsts: null
  https: true
  tls_version: TLSv1.2
kind: domain-security
layout: security
method: probed
name: Shanghai Jiao Tong University Domain Security
name_suffix: Domain Security
overview: 'Domain security posture for Shanghai Jiao Tong University, probed live across 3 host(s) and 1 registrable domain(s). 3 host(s) serve HTTPS (up to TLSv1.2); 0 advertise HSTS. Email/DNS controls: DNSSEC present, SPF present, DMARC present (p=none).'
provider_name: Shanghai Jiao Tong University
provider_slug: shanghai-jiao-tong-university
slug: shanghai-jiao-tong-university-domain-security
source_filename: shanghai-jiao-tong-university-domain-security.yml
source_heading: Domain Security
source_url: ''
source_yaml: "generated: '2026-07-11'\nmethod: probed\nsource: live DNS/TLS/HTTP probes of apis.yml + OpenAPI hosts\nhosts:\n- host: en.sjtu.edu.cn\n  https: true\n  tls_version: TLSv1.2\n  cert_expires: Oct 18 23:59:59 2026 GMT\n  hsts: false\n- host: developer.sjtu.edu.cn\n  https: true\n  tls_version: TLSv1.2\n  cert_expires: Oct 18 23:59:59 2026 GMT\n  hsts: false\n- host: jaccount.sjtu.edu.cn\n  https: true\n  tls_version: TLSv1.2\n  cert_expires: Oct 18 23:59:59 2026 GMT\n  hsts: null\ndomains:\n- domain: sjtu.edu.cn\n  dnssec: true\n  caa: []\n  spf: true\n  dmarc: true\n  dmarc_policy: none\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/shanghai-jiao-tong-university/refs/heads/main/security/shanghai-jiao-tong-university-domain-security.yml
summary_line: TLSv1.2 · DNSSEC · DMARC
tags:
- Education
- Higher Education
- University
- Research
- China
- GraphQL
- Identity
- OpenID Connect
---
