...
oauth_flows: []
overview: ElevenLabs secures its APIs with apiKey, oauth2, and bearer across 3 declared security schemes, as derived from its OpenAPI definitions.
...
- applies_to: the whole REST API surface (api.elevenlabs.io and the regional residency hosts)
  ...
  - control: IP allowlisting
    description: a key can be bound to specific IP addresses or CIDR ranges; requests from other addresses are rejected with 403.
    docs: https://elevenlabs.io/docs/overview/administration/workspaces/api-keys#ip-allowlisting
  description: 'The ElevenLabs API key, sent on every REST request as `xi-api-key: <key>`. The docs
...
- applies_to: client-side realtime surfaces (agents/conversation tokens, signed URLs)
  description: 'Short-lived single-use tokens for endpoints that need to be called from a client
...
- authorization_endpoint: https://elevenlabs.io/app/oauth/authorize
  ...
  - S256
  description: 'OAuth 2.0 with PKCE, published as RFC 8414 authorization-server metadata. Used by the
