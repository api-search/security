...
oauth_flows: []
overview: beehiiv secures its APIs with http and oauth2 across 3 declared security schemes, as derived from its OpenAPI definitions.
provider_name: beehiiv
provider_slug: beehiiv
scheme_count: 3
schemes:
- applies_to: All 95 operations of the v2 REST API at https://api.beehiiv.com/v2
  description: 'Publication API key created in the beehiiv app (Settings > API). Sent as Authorization: Bearer <api key>.'
  ...
  scheme: bearer
  scoping: A key is scoped to the workspace/organization; the 180 req/min rate limit is shared across every key in that organization.
  sources:
  - openapi/_original/beehiiv-openapi.yml
  type: http
- authorizationUrl: https://app.beehiiv.com/oauth/authorize
  client_registration: Manual — contact beehiiv Support to register an OAuth application. No dynamic client registration on the REST API.
  description: 'Standard OAuth2 authorization-code flow for third-party integrations. Tokens are used as the same Authorization: Bearer header against api.beehiiv.com.'
...
- applies_to: https://mcp.beehiiv.com/mcp
  ...
  description: Separate OAuth 2.1 authorization server fronting the beehiiv MCP, discoverable via RFC 8414 metadata. Distinct issuer and a coarser scope vocabulary from the REST OAuth above.
  flow: authorizationCode
