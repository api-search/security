...
scheme_count: 1
schemes:
- description: 'Pass API key as a Bearer token in the Authorization header (e.g. Authorization: Bearer [example key]). When using this method, omit the API key from the URL path. The path-key endpoints do not use a security scheme in the OpenAPI sense; the {apiKey} path parameter serves as the credential.'
  name: BearerAuth
  scheme: bearer
  sources:
