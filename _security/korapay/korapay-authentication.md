...
scheme_count: 4
schemes:
- description: 'Secret API key (sk_...) passed as `Authorization: Bearer sk_YOUR_SECRET_KEY`. Required for server-side and financial operations - payouts, balances, virtual bank accounts, refunds, conversions, and charge verification. Keep confidential; never expose in client-side code. Test and Live modes use different secret keys.'
  name: secretKeyAuth
  scheme: bearer
  sources:
