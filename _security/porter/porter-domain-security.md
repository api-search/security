---
api_specs:
- filename: porter-bundle-openapi.yml
  format: yaml
  label: Porter Bundle API
  slug: porter-bundle-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/porter/refs/heads/main/openapi/porter-bundle-openapi.yml
description: ''
domains:
- caa: []
  dmarc: false
  dnssec: false
  domain: porter.sh
  spf: true
hosts:
- cert_expires: Sep  7 19:17:52 2026 GMT
  host: porter.sh
  hsts: true
  hsts_max_age: 31536000
  https: true
  tls_version: TLSv1.3
kind: domain-security
layout: security
method: probed
name: Porter Domain Security
name_suffix: Domain Security
overview: 'Domain security posture for Porter, probed live across 1 host(s) and 1 registrable domain(s). 1 host(s) serve HTTPS (up to TLSv1.3); 1 advertise HSTS. Email/DNS controls: DNSSEC absent, SPF present, DMARC absent.'
provider_name: Porter
provider_slug: porter
slug: porter-domain-security
source_filename: porter-domain-security.yml
source_heading: Domain Security
source_url: ''
source_yaml: "generated: '2026-07-11'\nmethod: probed\nsource: live DNS/TLS/HTTP probes of apis.yml + OpenAPI hosts\nhosts:\n- host: porter.sh\n  https: true\n  tls_version: TLSv1.3\n  cert_expires: Sep  7 19:17:52 2026 GMT\n  hsts: true\n  hsts_max_age: 31536000\ndomains:\n- domain: porter.sh\n  dnssec: false\n  caa: []\n  spf: true\n  dmarc: false\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/porter/refs/heads/main/security/porter-domain-security.yml
summary_line: TLSv1.3 · HSTS
tags:
- Cloud Native
- CNAB
- DevOps
- Kubernetes
- Package Manager
---
