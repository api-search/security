...
scheme_count: 1
schemes:
- description: 'Bearer token authentication. Tabby uses a registration token from the self-hosted web UI (Information -> System -> Registration token), passed as `Authorization: Bearer YOUR_TOKEN`. Because Tabby is self-hosted, tokens are minted by your own instance rather than a central vendor console.'
  name: bearerAuth
  scheme: bearer
  sources:
