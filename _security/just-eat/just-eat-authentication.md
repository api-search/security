---
api_key_in:
- header
api_specs:
- filename: just-eat-attempted-delivery-api-api-openapi.yml
  format: yaml
  label: Just Eat Attempted Delivery API API
  slug: just-eat-attempted-delivery-api-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-attempted-delivery-api-api-openapi.yml
- filename: just-eat-attempted-delivery-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat Attempted Delivery Webhooks API
  slug: just-eat-attempted-delivery-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-attempted-delivery-webhooks-api-openapi.yml
- filename: just-eat-checkout-api-openapi.yml
  format: yaml
  label: Just Eat Checkout API
  slug: just-eat-checkout-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-checkout-api-openapi.yml
- filename: just-eat-consumer-queries-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat Consumer Queries Webhooks API
  slug: just-eat-consumer-queries-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-consumer-queries-webhooks-api-openapi.yml
- filename: just-eat-consumerqueries-api-openapi.yml
  format: yaml
  label: Just Eat ConsumerQueries API
  slug: just-eat-consumerqueries-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-consumerqueries-api-openapi.yml
- filename: just-eat-consumers-api-openapi.yml
  format: yaml
  label: Just Eat Consumers API
  slug: just-eat-consumers-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-consumers-api-openapi.yml
- filename: just-eat-delivery-pools-api-api-openapi.yml
  format: yaml
  label: Just Eat Delivery Pools API API
  slug: just-eat-delivery-pools-api-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-delivery-pools-api-api-openapi.yml
- filename: just-eat-deliveryfee-api-openapi.yml
  format: yaml
  label: Just Eat DeliveryFee API
  slug: just-eat-deliveryfee-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-deliveryfee-api-openapi.yml
- filename: just-eat-order-acceptance-api-api-openapi.yml
  format: yaml
  label: Just Eat Order Acceptance API API
  slug: just-eat-order-acceptance-api-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-order-acceptance-api-api-openapi.yml
- filename: just-eat-order-acceptance-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat Order Acceptance Webhooks API
  slug: just-eat-order-acceptance-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-order-acceptance-webhooks-api-openapi.yml
- filename: just-eat-order-api-api-openapi.yml
  format: yaml
  label: Just Eat Order API API
  slug: just-eat-order-api-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-order-api-api-openapi.yml
- filename: just-eat-order-delivery-api-api-openapi.yml
  format: yaml
  label: Just Eat Order Delivery API API
  slug: just-eat-order-delivery-api-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-order-delivery-api-api-openapi.yml
- filename: just-eat-order-delivery-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat Order Delivery Webhooks API
  slug: just-eat-order-delivery-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-order-delivery-webhooks-api-openapi.yml
- filename: just-eat-order-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat Order Webhooks API
  slug: just-eat-order-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-order-webhooks-api-openapi.yml
- filename: just-eat-publicly-accessible-api-openapi.yml
  format: yaml
  label: Just Eat publicly-accessible API
  slug: just-eat-publicly-accessible-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-publicly-accessible-api-openapi.yml
- filename: just-eat-restaurant-claims-api-openapi.yml
  format: yaml
  label: Just Eat Restaurant Claims API
  slug: just-eat-restaurant-claims-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurant-claims-api-openapi.yml
- filename: just-eat-restaurant-events-api-openapi.yml
  format: yaml
  label: Just Eat Restaurant Events API
  slug: just-eat-restaurant-events-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurant-events-api-openapi.yml
- filename: just-eat-restaurant-events-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat Restaurant Events Webhooks API
  slug: just-eat-restaurant-events-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurant-events-webhooks-api-openapi.yml
- filename: just-eat-restaurant-ordertimes-api-openapi.yml
  format: yaml
  label: Just Eat Restaurant OrderTimes API
  slug: just-eat-restaurant-ordertimes-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurant-ordertimes-api-openapi.yml
- filename: just-eat-restaurant-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat Restaurant Webhooks API
  slug: just-eat-restaurant-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurant-webhooks-api-openapi.yml
- filename: just-eat-restaurantqueries-api-openapi.yml
  format: yaml
  label: Just Eat RestaurantQueries API
  slug: just-eat-restaurantqueries-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurantqueries-api-openapi.yml
- filename: just-eat-restaurantqueries-webhooks-api-openapi.yml
  format: yaml
  label: Just Eat RestaurantQueries Webhooks API
  slug: just-eat-restaurantqueries-webhooks-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurantqueries-webhooks-api-openapi.yml
- filename: just-eat-restaurants-api-openapi.yml
  format: yaml
  label: Just Eat Restaurants API
  slug: just-eat-restaurants-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-restaurants-api-openapi.yml
- filename: just-eat-search-api-openapi.yml
  format: yaml
  label: Just Eat Search API
  slug: just-eat-search-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/openapi/just-eat-search-api-openapi.yml
auth_types:
- apiKey
- http
- openIdConnect
description: ''
kind: authentication
layout: security
method: derived
name: Just Eat Authentication
name_suffix: Authentication
oauth_flows: []
overview: Just Eat secures its APIs with apiKey, http, and openIdConnect across 4 declared security schemes, as derived from its OpenAPI definitions.
provider_name: Just Eat
provider_slug: just-eat
scheme_count: 4
schemes:
- bearerFormat: JWT
  description: Bearer token authentication using a JSON Web Token (JWT).
  name: Bearer
  scheme: bearer
  sources:
  - openapi/just-eat-uk-openapi-original.json
  type: http
- description: APIs for delivery partners require an API key that will have been issued to you.
  in: header
  name: api_key
  parameter: Authorization
  sources:
  - openapi/just-eat-uk-openapi-original.json
  type: apiKey
- description: A few services allow the use of basic authentication when a partner doesn't support OAuth based authentication.
  name: basic_auth
  scheme: basic
  sources:
  - openapi/just-eat-uk-openapi-original.json
  type: http
- description: ADFS authentication provider for internal Just Eat tools.
  name: justeat_adfs
  openIdConnectUrl: https://adfs.just-eat.com/adfs/.well-known/openid-configuration
  sources:
  - openapi/just-eat-uk-openapi-original.json
  type: openIdConnect
slug: just-eat-authentication
source_filename: just-eat-authentication.yml
source_heading: Authentication Profile
source_url: ''
source_yaml: "generated: '2026-07-19'\nmethod: derived\nsource: openapi/just-eat-uk-openapi-original.json\nsummary:\n  types:\n  - apiKey\n  - http\n  - openIdConnect\n  api_key_in:\n  - header\nschemes:\n- name: Bearer\n  type: http\n  scheme: bearer\n  bearerFormat: JWT\n  description: Bearer token authentication using a JSON Web Token (JWT).\n  sources:\n  - openapi/just-eat-uk-openapi-original.json\n- name: api_key\n  type: apiKey\n  in: header\n  parameter: Authorization\n  description: APIs for delivery partners require an API key that will have been issued to you.\n  sources:\n  - openapi/just-eat-uk-openapi-original.json\n- name: basic_auth\n  type: http\n  scheme: basic\n  description: A few services allow the use of basic authentication when a partner doesn't support\n    OAuth based authentication.\n  sources:\n  - openapi/just-eat-uk-openapi-original.json\n- name: justeat_adfs\n  type: openIdConnect\n  openIdConnectUrl: https://adfs.just-eat.com/adfs/.well-known/openid-configuration\n\
  \  description: ADFS authentication provider for internal Just Eat tools.\n  sources:\n  - openapi/just-eat-uk-openapi-original.json\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/just-eat/refs/heads/main/authentication/just-eat-authentication.yml
summary_line: apiKey/http/openIdConnect · 4 schemes
tags:
- Food Delivery
- Restaurant
- Marketplace
- Logistics
- Order
- Delivery
- Point-of-Sale
- E-Commerce
---
