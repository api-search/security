---
api_specs:
- filename: gray-swan-activity-api-openapi.yml
  format: yaml
  label: Gray Swan Activity API
  slug: gray-swan-activity-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/gray-swan/refs/heads/main/openapi/gray-swan-activity-api-openapi.yml
- filename: gray-swan-cygnal-api-openapi.yml
  format: yaml
  label: Gray Swan Cygnal API
  slug: gray-swan-cygnal-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/gray-swan/refs/heads/main/openapi/gray-swan-cygnal-api-openapi.yml
- filename: gray-swan-policies-api-openapi.yml
  format: yaml
  label: Gray Swan Policies API
  slug: gray-swan-policies-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/gray-swan/refs/heads/main/openapi/gray-swan-policies-api-openapi.yml
certifications:
- SOC 2 Type 2
- Cyber Essentials
description: ''
kind: trust-center
layout: security
name: Gray Swan Trust Center
name_suffix: Trust Center
overview: Gray Swan maintains a public trust center documenting SOC 2 Type 2 and Cyber Essentials compliance.
provider_name: Gray Swan
provider_slug: gray-swan
slug: gray-swan-trust-center
source_filename: gray-swan-trust-center.yml
source_heading: Trust Center
source_url: ''
source_yaml: "generated: '2026-07-19'\nmethod: searched\nprobe: true\nsource: https://trust.grayswan.ai/\nurl: https://trust.grayswan.ai/\nplatform: SafeBase\ncertifications:\n- SOC 2 Type 2\n- Cyber Essentials\nevidence:\n- source: https://trust.grayswan.ai/\n  keywords:\n  - soc 2 type 2\n  - cyber essentials\n  - trust center\n  - compliance certification\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/gray-swan/refs/heads/main/security/gray-swan-trust-center.yml
summary_line: SOC 2 Type 2, Cyber Essentials
tags:
- Company
- Artificial Intelligence
- AI Security
- LLM Security
- Guardrails
- Red Teaming
- AI Governance
- Prompt Injection
- Model Safety
- API Security
trust_url: https://trust.grayswan.ai/
---
