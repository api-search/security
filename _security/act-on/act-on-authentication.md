...
oauth_flows: []
overview: Act-On declares 3 security scheme(s) across its OpenAPI definitions.
...
- flows:
  ...
  - description: Exchange an authorization code for an access_token and refresh_token.
    ...
    in_spec: false
    note: Documented on the developer portal but absent from the published oauth OpenAPI document.
  name: oauth2
...
- bearer_format: bearer
  ...
  in: header
  name: sec0
  note: Declared as `apiKey` in the header rather than `http`/`bearer`. It is not a separate credential — it is the OAuth bearer token modelled loosely by the ReadMe tooling. There is NO API-key-only authentication path on Act-On.
  spec_source: openapi/act-on-rest-api-openapi.yml, openapi/act-on-raw-body-api-openapi.yml
  type: apiKey
- bearerFormat: JWT
  name: bearerAuth
  note: The Custom Objects Service models the same bearer token correctly, and states the format is JWT.
  scheme: bearer
