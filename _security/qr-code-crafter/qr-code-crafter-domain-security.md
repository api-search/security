---
api_specs:
- filename: openapi.json
  format: json
  label: QRCodeCrafter API
  slug: qrcodecrafter-api
  spec_type: OpenAPI
  url: https://qrcodecrafter.com/.well-known/openapi.json
description: ''
domains:
- caa: []
  dmarc: false
  dnssec: false
  domain: qrcodecrafter.com
  spf: false
hosts:
- cert_expires: Oct 10 16:18:43 2026 GMT
  host: qrcodecrafter.com
  hsts: true
  hsts_max_age: 31536000
  https: true
  tls_version: TLSv1.3
kind: domain-security
layout: security
method: probed
name: Qr Code Crafter Domain Security
name_suffix: Domain Security
overview: 'Domain security posture for QR Code Crafter, probed live across 1 host(s) and 1 registrable domain(s). 1 host(s) serve HTTPS (up to TLSv1.3); 1 advertise HSTS. Email/DNS controls: DNSSEC absent, SPF absent, DMARC absent.'
provider_name: QR Code Crafter
provider_slug: qr-code-crafter
slug: qr-code-crafter-domain-security
source_filename: qr-code-crafter-domain-security.yml
source_heading: Domain Security
source_url: ''
source_yaml: "generated: '2026-08-11'\nmethod: probed\nsource: live DNS/TLS/HTTP probes of apis.yml + OpenAPI hosts\nhosts:\n- host: qrcodecrafter.com\n  https: true\n  tls_version: TLSv1.3\n  cert_expires: Oct 10 16:18:43 2026 GMT\n  hsts: true\n  hsts_max_age: 31536000\ndomains:\n- domain: qrcodecrafter.com\n  dnssec: false\n  caa: []\n  spf: false\n  dmarc: false\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/qr-code-crafter/refs/heads/main/security/qr-code-crafter-domain-security.yml
summary_line: TLSv1.3 · HSTS
tags:
- QR code
- QR code generation
- static QR
- dynamic QR
- dynamic redirects
- image export
- bulk generation
- developer tools
- OpenAPI
- WebMCP
- browser agents
- payments (QR)
- agent readiness
- llms.txt
- capability tokens
---
