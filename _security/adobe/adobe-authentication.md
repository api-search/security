...
oauth_flows:
- clientCredentials
overview: Adobe secures its APIs with http, apiKey, and oauth2 across 3 declared security schemes, as derived from its OpenAPI definitions. OAuth 2.0 is offered via the clientCredentials flow(s).
provider_name: Adobe
provider_slug: adobe
scheme_count: 3
schemes:
- bearerFormat: JWT
  description: OAuth 2.0 access token obtained via the client credentials grant from Adobe IMS. Generate credentials in the Adobe Developer Console and exchange them at https://ims-na1.adobelogin.com/ims/token/v3.
  in: header
  name: bearerAuth
  operations: 48
...
- description: Declared as a required header parameter on 48 of 49 operations in the official spec (every operation except POST /token). Not optional and not redundant with the bearer token.
  in: header
  name: apiKeyAuth
  operations: 48
  parameter: x-api-key
...
- description: '"A user token referencing the user''s individual account, obtained using their credentials." Declared on the Substance 3D operations in openapi/_original/adobe-substance-3d-api-openapi.yml.'
  in: header
  name: X-User-Token
