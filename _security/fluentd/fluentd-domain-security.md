---
api_specs:
- filename: fluentd-forward-protocol-asyncapi.yml
  format: yaml
  label: Fluentd Forward Protocol
  slug: fluentd-forward-protocol
  spec_type: AsyncAPI
  url: https://raw.githubusercontent.com/api-evangelist/fluentd/refs/heads/main/asyncapi/fluentd-forward-protocol-asyncapi.yml
- filename: fluentd-http-input-openapi.yml
  format: yaml
  label: Fluentd HTTP Input API
  slug: fluentd-http-input
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/fluentd/refs/heads/main/openapi/fluentd-http-input-openapi.yml
description: ''
domains:
- caa: []
  dmarc: false
  dnssec: false
  domain: fluentd.org
  spf: false
hosts:
- cert_expires: Sep 17 23:08:20 2026 GMT
  host: www.fluentd.org
  hsts: true
  hsts_max_age: 31536000
  https: true
  tls_version: TLSv1.3
- cert_expires: Sep  3 02:25:55 2026 GMT
  host: docs.fluentd.org
  hsts: true
  hsts_max_age: 31536000
  https: true
  tls_version: TLSv1.3
kind: domain-security
layout: security
method: probed
name: Fluentd Domain Security
name_suffix: Domain Security
overview: 'Domain security posture for Fluentd, probed live across 2 host(s) and 1 registrable domain(s). 2 host(s) serve HTTPS (up to TLSv1.3); 2 advertise HSTS. Email/DNS controls: DNSSEC absent, SPF absent, DMARC absent.'
provider_name: Fluentd
provider_slug: fluentd
slug: fluentd-domain-security
source_filename: fluentd-domain-security.yml
source_heading: Domain Security
source_url: ''
source_yaml: "generated: '2026-07-11'\nmethod: probed\nsource: live DNS/TLS/HTTP probes of apis.yml + OpenAPI hosts\nhosts:\n- host: www.fluentd.org\n  https: true\n  tls_version: TLSv1.3\n  cert_expires: Sep 17 23:08:20 2026 GMT\n  hsts: true\n  hsts_max_age: 31536000\n- host: docs.fluentd.org\n  https: true\n  tls_version: TLSv1.3\n  cert_expires: Sep  3 02:25:55 2026 GMT\n  hsts: true\n  hsts_max_age: 31536000\ndomains:\n- domain: fluentd.org\n  dnssec: false\n  caa: []\n  spf: false\n  dmarc: false\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/fluentd/refs/heads/main/security/fluentd-domain-security.yml
summary_line: TLSv1.3 · HSTS
tags:
- Data Collection
- Logging
- Open Source
---
