---
description: ''
domains:
- caa: []
  dmarc: true
  dmarc_policy: none
  dnssec: false
  domain: bancomat.it
  spf: true
hosts:
- cert_expires: Sep 18 19:28:22 2026 GMT
  host: bancomat.it
  hsts: true
  hsts_max_age: 31536000
  https: true
  tls_version: TLSv1.3
kind: domain-security
layout: security
method: probed
name: Bancomat Domain Security
name_suffix: Domain Security
overview: 'Domain security posture for Bancomat, probed live across 1 host(s) and 1 registrable domain(s). 1 host(s) serve HTTPS (up to TLSv1.3); 1 advertise HSTS. Email/DNS controls: DNSSEC absent, SPF present, DMARC present (p=none).'
provider_name: Bancomat
provider_slug: bancomat
slug: bancomat-domain-security
source_filename: bancomat-domain-security.yml
source_heading: Domain Security
source_url: ''
source_yaml: "generated: '2026-07-11'\nmethod: probed\nsource: live DNS/TLS/HTTP probes of apis.yml + OpenAPI hosts\nhosts:\n- host: bancomat.it\n  https: true\n  tls_version: TLSv1.3\n  cert_expires: Sep 18 19:28:22 2026 GMT\n  hsts: true\n  hsts_max_age: 31536000\ndomains:\n- domain: bancomat.it\n  dnssec: false\n  caa: []\n  spf: true\n  dmarc: true\n  dmarc_policy: none\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/bancomat/refs/heads/main/security/bancomat-domain-security.yml
summary_line: TLSv1.3 · HSTS · DMARC
tags:
- ATM
- Banking
- Financial Services
- Italy
- Mobile Payments
- Payments
- Debit Cards
---
