---
api_specs:
- filename: national-geospatial-intelligence-agency-openapi.json
  format: json
  label: NGA Earth-Info REST API
  slug: earth-info-rest-api
  spec_type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/national-geospatial-intelligence-agency/main/openapi/national-geospatial-intelligence-agency-openapi.json
description: ''
domains:
- caa: []
  dmarc: true
  dmarc_policy: reject
  dnssec: true
  domain: nga.mil
  spf: true
hosts:
- cert_expires: Feb 19 22:38:24 2027 GMT
  host: www.nga.mil
  hsts: true
  hsts_max_age: 63072000
  https: true
  tls_version: TLSv1.3
- cert_expires: Dec  2 20:31:42 2026 GMT
  host: earth-info.nga.mil
  hsts: true
  hsts_max_age: 31536000
  https: true
  tls_version: TLSv1.2
kind: domain-security
layout: security
method: probed
name: National Geospatial Intelligence Agency Domain Security
name_suffix: Domain Security
overview: 'Domain security posture for National Geospatial-Intelligence Agency, probed live across 2 host(s) and 1 registrable domain(s). 2 host(s) serve HTTPS (up to TLSv1.3); 2 advertise HSTS. Email/DNS controls: DNSSEC present, SPF present, DMARC present (p=reject).'
provider_name: National Geospatial-Intelligence Agency
provider_slug: national-geospatial-intelligence-agency
slug: national-geospatial-intelligence-agency-domain-security
source_filename: national-geospatial-intelligence-agency-domain-security.yml
source_heading: Domain Security
source_url: ''
source_yaml: "generated: '2026-07-11'\nmethod: probed\nsource: live DNS/TLS/HTTP probes of apis.yml + OpenAPI hosts\nhosts:\n- host: www.nga.mil\n  https: true\n  tls_version: TLSv1.3\n  cert_expires: Feb 19 22:38:24 2027 GMT\n  hsts: true\n  hsts_max_age: 63072000\n- host: earth-info.nga.mil\n  https: true\n  tls_version: TLSv1.2\n  cert_expires: Dec  2 20:31:42 2026 GMT\n  hsts: true\n  hsts_max_age: 31536000\ndomains:\n- domain: nga.mil\n  dnssec: true\n  caa: []\n  spf: true\n  dmarc: true\n  dmarc_policy: reject\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/national-geospatial-intelligence-agency/refs/heads/main/security/national-geospatial-intelligence-agency-domain-security.yml
summary_line: TLSv1.3 · HSTS · DNSSEC · DMARC
tags:
- Federal Government
- Geospatial
- Intelligence
- Defense
- Geomatics
---
