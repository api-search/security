...
scheme_count: 2
schemes:
- description: 'Secret integration key for the Checkout API (https://payments.yoco.com/api), passed as `Authorization: Bearer [example key]` (live) or `Authorization: Bearer [example key]` (test). Keys are self-served from the Yoco app under Sales -> Payment Gateway. The Checkout API must be called server-side so the secret key is never exposed to the browser.'
  name: secretKey
  scheme: bearer
  sources:
